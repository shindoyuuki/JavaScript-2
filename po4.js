function update(a){  /*input tag を更新する関数*/
    document.querySelector('input').value = a  /*value(値)*  +=は「追加」。 = は「上書き」「更新」の意味になります。
なの                                                         で、+= '1'と書くと、[ボタン]がクリックされるたびに1の文字が追加され、= ''と書くと、valueがクリアされ、表示内容が空っぽになります。*/
    }
        
function yuuki(b){ /*数字ボタンが押されたので数字を後ろに追加する*/
    document.querySelector('input').value += b /*+=にすることで、数字を入力した時に横に並んで表示される*/
    }
